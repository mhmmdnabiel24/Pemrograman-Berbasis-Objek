...
if __name__ == "__main__":
    ...
    # 2. CHALLENGE: Mengganti CashPayment dengan DebitCardPayment
    # Perhatikan bahwa kita hanya mengubah baris ini saja
    from services import DebitCardPayment 
    payment_method = DebitCardPayment() 
    
    # 3. Inject Dependencies (PosApp tetap menggunakan payment_method yang baru)
    # Ini membuktikan fleksibilitas sistem karena tidak mengubah isi class PosApp
    app = PosApp(repository=repo, payment_processor=payment_method)
    
    
    # Loop CLI untuk interaksi
    while True:
        print("\nMenu:")
